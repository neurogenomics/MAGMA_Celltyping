...
magma.tileplot <- function(ctd, results, height = 13, width = 4, annotLevel = 1, fileTag = "", output_path) {
    ...
    if (sum(!c("Celltype", "GCOV_FILE", "log10p", "level") %in% colnames(results)) > 0) {
        stop("results dataframe must contain 'Celltype', 'GCOV_FILE', 'log10p' and 'level' columns")
    }
    if (length(unique(results$GCOV_FILE)) < 2) {
        stop("Must be more than one unique entry in results$GCOV_FILE for plotting tileplot")
    }
    if (!annotLevel %in% results$level) {
        stop(sprintf("No results for annotation level = %s found in results", annotLevel))
    }
    ...
    fig_Heatmap_WOdendro <- ggplot(results) +
        geom_tile(aes_string(x = "GCOV_FILE", y = "Celltype", fill = "log10p")) +
        ...
        ggtitle("MAGMA") +
        geom_point(aes_string(x = "GCOV_FILE", y = "Celltype", size = ifelse(q < 0.00001, "HUGEdot", ifelse(q < 0.0001, "BIGdot", ifelse(q < 0.001, "BiiGdot", ifelse(q < 0.05, "dot", "no_dot"))))), col = "black") +
    ...
    grDevices::pdf(sprintf("%s/CombinedRes_TilePlot_MAGMA_noDendro_level%s_%s.pdf", figurePath, annotLevel, fileTag), width = width, height = height)
    print(fig_Heatmap_WOdendro)
    grDevices::dev.off()

    grDevices::pdf(sprintf("%s/CombinedRes_TilePlot_MAGMA_wtDendro_level%s_%s.pdf", figurePath, annotLevel, fileTag), width = width + 1, height = height)
    print(gridExtra::grid.arrange(fig_Heatmap_WOdendro, Fig_Dendro, ncol = 2, widths = c(0.8, 0.2)))
    grDevices::dev.off()
