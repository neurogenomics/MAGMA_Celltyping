...
magma.tileplot <- function(ctd,results,height=13,width=4,annotLevel=1,fileTag="",output_path){
    ...
    fig_Heatmap_WOdendro = ggplot(results)+geom_tile(aes_string(x="GCOV_FILE",y="Celltype",fill="log10p")) + 
    ...
    Fig_Dendro <- ggplot(ggdendro::segment(ctdDendro$ddata)) + geom_segment(aes_string(x="x", y="y", xend="xend", yend="yend")) + coord_flip() +  theme_dendro()
