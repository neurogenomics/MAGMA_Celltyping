...
calculate_conditional_celltype_enrichment_probabilities_wtLimma <- function(
    magma1,
    magma2,
    ctd,
    thresh = 0.0001,
    ...
    for (ct1 in allCellTypes) {
        ...
        if (sctSpecies == "mouse") {
            ...
            magma_with_ct1 <- geneGroups %>%
                dplyr::rename(mouse.symbol = mgi_symbol) %>%
            ...
            magma_with_ct1 <- geneGroups %>%
                dplyr::rename(human.symbol = hgnc_symbol) %>%
        ...
        mZ <- reshape::cast(
            data = magma_with_ct1,
            formula = mouse.symbol ~ variable,
            fun.aggregate = mean
        )
        mZ2 <- merge(
            x = mZ,
            y = magma_with_ct1[, c("mouse.symbol", "percentile")],
            by = "mouse.symbol"
        )
        ...
        ## lmer is an exportedfunction from both lme4, lmerTest
        ## Using the lme4 implementation here 
    ...
    ggplot(df3) +
        geom_bar(aes(x = ct, y = log10p, fill = variable),
...
calculate.conditional.celltype.enrichment.probabilities.wtLimma <-
    function(...) {
        .Deprecated(
            "calculate_conditional_celltype_enrichment_probabilities_wtLimma"
        )
        calculate_conditional_celltype_enrichment_probabilities_wtLimma(...)
    }
