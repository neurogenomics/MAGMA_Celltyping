...
plot_celltype_associations <- function(ctAssocs, ctd, useSignificanceLine = TRUE, savePDF = TRUE, fileTag = "", plotDendro = TRUE, gwas_title = "", plotLegend = TRUE, figsDir = NA) {
    ...
    for (annotLevel in 1:sum(names(ctAssocs) == "")) {
        ...
        a2 <- ggplot(ctAssocs[[annotLevel]]$results, aes_string(x = "factor(Celltype)", y = "log10p", fill = "FullMethod")) +
            scale_y_reverse() +
        ...
        if (useSignificanceLine) {
            a2 <- a2 + geom_hline(yintercept = log(as.numeric(0.05 / ctAssocs$total_baseline_tests_performed), 10), colour = "black")
        }
        ...
        if (length(unique(ctAssocs[[1]]$results$CONTROL)) == 1) {
            if (plotDendro == TRUE) {
                theFig <- grid.arrange(a2, ctdDendro$dendroPlot, ncol = 2, widths = c(0.8, 0.2))
            }
