...
get_genomebuild_for_sumstats <- function(path){
  ...
  topLines = 
    utils::read.table(path,nrows=30000,header = TRUE,stringsAsFactors = FALSE)
  ...
  #If no matches found in the sampled reference dataset use full
  if(nrow(topSNP_locs)==0)
    topSNP_locs = SNP_LOC_DATA[SNP_LOC_DATA$SNP %in% topLines$SNP,]
  #if still none found there is an issue
  if(nrow(topSNP_locs)==0)
    stop(paste0("No SNPs from the inputted summary statistics file match ",
                  "reference data. Please check SNP ID's are correct."))
  topSNP_locs$locs = 
    sprintf("%s-%s-%s",topSNP_locs$SNP,topSNP_locs$CHR,topSNP_locs$BP)
