...
get.ctd.dendro <- function(ctd,annotLevel){
  ...
  #library(gridExtra)
  # Prepare dendrogram
  binned_file_dist <- dist(t(ctd[[annotLevel]]$specificity_quantiles)) # euclidean distances between the rows
  ...
  dendroPlot <- ggplot2::ggplot(segment(ddata)) + ggplot2::geom_segment(aes(x=x, y=y, xend=xend, yend=yend)) + ggplot2::coord_flip() +  ggdendro::theme_dendro() + ggplot2::scale_x_continuous(expand = c(0, 1.3)) 
