...
#'
#' @param geneLocFilePath The gene loc file as downloaded from MAGMA website (e.g. NCBI37.3.gene.loc)
#' @param geneLocFilePathOut The file path to write the gene loc file with MHC removed
#'
...
drop.genes.within.mhc <- function(geneLocFilePath,geneLocFilePathOut){
    ...
    utils::write.table(ncbi,file=geneLocFilePathOut,quote=FALSE,row.names = FALSE, col.names=F, sep="\t")
}
