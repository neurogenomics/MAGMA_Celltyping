...
calculate_conditional_geneset_enrichment <- function(geneset, ctd, controlledAnnotLevel = 1, controlledCTs, gwas_sumstats_path, analysis_name, upstream_kb = 10, downstream_kb = 1.5, genome_ref_path, geneset_species = "mouse", specificity_species) {
    ...
    for (i in 1:length(controlledCTs)) {
        ...
        if (!reqCT %in% annotLvlCTs) {
            stop(sprintf("User requested the following cell type be controlled for but it cannot be found: %s from annotation level %s", reqCT, controlledAnnotLevel))
        }
    }
    ...
    magma_cmd_cond <- sprintf("magma --gene-results '%s.genes.raw' --set-annot '%s' --gene-covar '%s' --model direction=positive condition='%s' --out '%s.%s'", magmaPaths$filePathPrefix, geneSetFile, geneCovarFile, ctrldCTs, magmaPaths$filePathPrefix, analysis_name)
