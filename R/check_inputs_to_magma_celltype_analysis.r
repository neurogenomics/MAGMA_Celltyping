...
check_inputs_to_magma_celltype_analysis <- function(ctd, gwas_sumstats_path, analysis_name, upstream_kb, downstream_kb, genome_ref_path) {
    ...
    for (annotLevel in 1:length(ctd)) {
        # if(!"quantiles" %in% names(ctd[[annotLevel]])){stop("CTD should have quantiles. Send to 'prepare_quantile_groups' before calling this function.")}
        if (!"specificity_quantiles" %in% names(ctd[[annotLevel]])) {
            stop("CTD should have quantiles. Send to 'prepare_quantile_groups' before calling this function.")
        }
    }
    # - Is annotLevel within length(ctd)
    if (!annotLevel %in% 1:length(ctd)) {
        stop("annotLevel does not correspond to a level available with ctd[[annotLevel]]")
    }
    ...
    if (!file.exists(sprintf("%s.genes.out", magmaPaths$filePathPrefix))) {
        stop(sprintf("%s.genes.out does not exist. Run map_snps_to_genes() before this function", magmaPaths$filePathPrefix))
    }
    #####################
}
