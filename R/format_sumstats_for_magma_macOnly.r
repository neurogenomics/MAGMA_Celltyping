...
format_sumstats_for_magma_macOnly <- function(path) {
    stop("This function is now depreciated. I stopped maintaining it in favour of the crossplatform version.")
    ...
    switch(Sys.info()[["sysname"]],
        Windows = {
            OS <- "Windows;print('This function will not work with Windows computers. Instead use format_sumstats_for_magma_crossplatform')"
        },
        Linux = {
            OS <- "Linux"
        },
    ...
    if (sum(c("CHR", "BP") %in% col_headers) == 2 & sum("SNP" %in% col_headers) == 0) {
        ...
        if (!genomebuild %in% c(1, 2)) {
            stop("Genome build must be entered as either 1 (for GRCh37) or 2 (for GRCh38)")
        }
