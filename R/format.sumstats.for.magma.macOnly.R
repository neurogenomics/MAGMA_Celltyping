...
format.sumstats.for.magma.macOnly <- function(path) {
    ...
    switch(Sys.info()[["sysname"]],
        Windows = {
            OS <- paste(
                "Windows;print('This function will not work with",
                "Windows computers.",
                "Instead use format.sumstats.for.magma')"
            )
        },
        Linux = {
            OS <- "Linux"
    ...
    con <- file(path, "r")
    row_of_data <- strsplit(readLines(con, n = 2)[2], "\t")[[1]]
    close(con)
    tmpPath <- tempfile()
    ...
    if (!sum(c("SNP", "BP") %in% col_headers) == 2) {
        # If not, see if there is a column storing the data 
        #.   in a format like: CHR:BP:A2:A1 or just CHR:BP
        # - UKBB data from Ben Neale has a Variant column with
        #.  CHR:POS:REF:ALT where ALT allele is the 
        #.   effect allele in the model [NB: the ALT allele is 
        #.   NOT always the minor allele]
        # -- For input to LDSC, A1 is effect allele, A2 is non-effect allele
        # - DIAGRAM diabetes data has a Chr:Position column with CHR:BP
        # - BMI adjusted for smoking has markername with CHR:BP 
        #    (with the chromosome name having 'chr' preceeding)
        # - Agression [EAGLE] just doesn't have any CHR or BP data
        print("Summary statistics file does not have obvious CHR or BP columns. 
        ...
        if (length(fourStepCol)) {
            # Convert the ':' into '\t'
            awkSplitCmd <- sprintf(
                "gawk -i inplace -F\":\" '$1=$1' OFS=\"\t\" %s", path, path)
            ...
            new_first_line <- gsub(curColName, "CHR\tBP\tA2\tA1\t",
                                   paste(col_headers, collapse = "\t"))
            sed_command <- sprintf("sed -i '' '1s/%s/%s/' %s",
                                   first_line, new_first_line, path)
        ...
        if (length(twoStepCol)) {
            # Convert the ':' into '\t'
            awkSplitCmd <- 
                sprintf("gawk -i inplace -F\":\" '$1=$1' OFS=\"\t\" %s",
                        path, path)
            system2("/bin/bash", args = c("-c", shQuote(awkSplitCmd)))
            # Replace the column name with four names
            ...
            # Write the new column headers to file
            first_line <- paste(col_headers, collapse = "\t")
            new_first_line <- gsub(curColName, "CHR\tBP", 
                                   paste(col_headers, collapse = "\t"))
            sed_command <- sprintf("sed -i '' '1s/%s/%s/' %s", 
                                   first_line, new_first_line, path)
            system2("/bin/bash", args = c("-c", shQuote(sed_command)))
    ...
    # If SNP is present... 
    # BUT not CHR or BP then need to find the relevant locations
    con <- file(path, "r")
    ...
    if (sum(c("CHR", "BP") %in% col_headers) == 0 & sum("SNP" %in% col_headers) == 1) {
        stop("I've blocked this function because I've not tested
             it since replacing SNP_LOC_DATA. 
             You should test it works manually. 
             Let me know if you test it!")
    }

    # If CHR and BP are present... 
    # BUT not SNP then need to find the relevant SNP ids
    con <- file(path, "r")
    rows_of_data <- readLines(con, n = 2)
    close(con)
    col_headers <- strsplit(rows_of_data[1], "\t")[[1]]
    if (sum(c("CHR", "BP") %in% col_headers) == 2 &
        ...
        if (!genomebuild %in% c(1, 2)) {
            stop("Genome build must be entered as either 1
                 (for GRCh37) or 2 (for GRCh38)")
        }
    ...
    if (sum(signed_stat_column_names %in% col_headers) < 1 %in% col_headers) {
        print("Header of file:")
        print(rows_of_data)
        stop("ERROR: cannot find a column name representing
             signed statistic in GWAS sumstats file. I.e. Z, OR, BETA")
    }
    ...
    if (!sum(col_headers[1:3] == c("SNP", "CHR", "BP")) == 3) {
        ...
        otherCols <- setdiff(1:length(col_headers),
                             c(whichSNP, whichCHR, whichBP)) 
        newColOrder <- sprintf("$%s", 
                               paste(c(whichSNP, whichCHR, whichBP, otherCols),
                                     collapse = " \"\\t\" $"))
        system(sprintf("gawk -i inplace '{ print %s}' %s", newColOrder, path))
    }

    # The formatting process can (rarely) result in duplicated columns, 
    # i.e. CHR, if CHR:BP is expanded and one already exists...
    # delete duplicates
    con <- file(path, "r")
    rows_of_data <- readLines(con, n = 2)
    close(con)
